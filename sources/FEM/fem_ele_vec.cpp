...
void CFiniteElementVec::GlobalAssembly_RHS()
{
	...
	if(Residual)
	{
		...
		case 0:                   // Liquid flow
			...
			for (i = 0; i < nnodes; i++)
			{
				val_n = h_pcs->GetNodeValue(nodes[i],idx_P1); // takes the pressure value from liquid flow to calculate fluid coupling
				...
				//std::fstream datei("K:\\Geomechanics\\Benchmark_Problem1\\OGS_Benchmarks\\Therzagi\\OGS-ECL\\klein\\two-way\\pressure_input_deformation_OGS.txt", std::ios::out | std::ios::app);
				//std::fstream datei("K:\\Geomechanics\\Benchmark_Problem1\\OGS_Benchmarks\\Therzagi\\OGS\\3D\\staggered\\klein\\OVERALL_COUPLING=3\\TwoWay_HM\\pressure_input_deformation_OGS.txt", std::ios::out | std::ios::app);
				//datei << val_n << std::endl;
				//datei.close();
...
double CFiniteElementVec::VolumeStrainIntegrationForEclipse()
{
	...
	for (int k = 0; k < nnodesHQ; k++) //Calculation of delta_u for t1-t0 for all High Order nodes 
	{
		...
		if (k == nnodesHQ - 1)
		{
			datei_ecl.open("K:\\Geomechanics\\Benchmark_Problem1\\OGS_Benchmarks\\Therzagi\\OGS-ECL\\klein\\two-way_XYZ\\displacement.txt", std::ios::out);
	...
	for (gp = 0; gp < nGauss; gp++)
	{
		...
		{
			for (int l = 0; l < nnodesHQ; l++) // du/dx + du/dy + du/dz
			{
				kl = nnodesHQ * n + l;
				dshp = dshapefctHQ[kl];
				if (n == 0 && axisymmetry)
