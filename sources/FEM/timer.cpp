...
CClockTime::CClockTime(void)
{
   delta_clocktime=0.0;
   time_flow.clear();
   time_mass_transport.clear();
   time_heat_transport.clear();
   time_mechanics.clear();
   time_kinreact.clear();
   time_equireact.clear();
   time_reactdeact.clear();
   time_timestep_number.clear();
   start = 0;
   end=0;
   time_total_flow = 0.0;
   time_total_transport = 0.0;
   time_total_heat = 0.0;
   time_total_mechanics = 0.0;
...
void CClockTime::StopTime(const std::string &name, unsigned int akt_timestep)
{
	char name1;
	name1 = name[0];
	long l=0;
	...
	//cout << " ClockTime: " << name << " :  " << delta_clocktime << ", TickTime: " << difftime << "\n";
	...
	case ('F'):
		...
		break;
	case ('T'):
		this->time_mass_transport.push_back(delta_clocktime);
	...
	case ('E'):
      ...
      case ('R'):
         ...
         break;
	  case ('H'):
		this->time_heat_transport.push_back(delta_clocktime);
		time_total_heat += delta_clocktime;
		break;
	  case ('M'):
		this->time_mechanics.push_back(delta_clocktime);
		time_total_mechanics += delta_clocktime;
		break;
	}
	// store timestep, but only if this has not been done before
	l = time_timestep_number.size();
	if(l==0) {
		time_timestep_number.push_back(akt_timestep);
		l++;}
	if(time_timestep_number[l-1] != akt_timestep)
		time_timestep_number.push_back(akt_timestep);
}
...
void CClockTime::PrintTimes(void)
{
   ...
   std::cout.precision(2);
    tot = time_total_flow + time_total_heat + time_total_mechanics + time_total_transport + time_total_kinreact + time_total_equireact + time_total_reactdeact;
    std::cout << "ClockTimes: " << "\n" <<
        "Unit      Flow:  Heat_Transport:    Mechanics:  Transport: KinReactions:  EquiReactions:  ReactDeact:      total: " << "\n";
    std::cout <<  "[sec] " << setw(9) << time_total_flow << "  " << setw(15) << time_total_heat << "  "
        << setw(12) << time_total_mechanics  << " " << setw(10) <<
    time_total_transport << "  " << setw(13) << time_total_kinreact << "  " << setw(14) <<
    time_total_equireact << "  " << setw(11) << time_total_reactdeact << "  " << setw(10) << tot <<  "\n";
    std::cout <<  "[%]   " << setw(9) << time_total_flow / tot * 100 << "  " << setw(15) << time_total_heat / tot * 100 << "  "
        << setw(12) << time_total_mechanics /tot * 100 << "  " <<  setw(10) <<
    time_total_transport / tot * 100 << "  " << setw(13) << time_total_kinreact / tot * 100 <<
    "  " <<  setw(14) << time_total_equireact / tot * 100 << "  " <<  setw(11) << time_total_reactdeact/tot*100<< "  " <<  setw(10) << tot/tot*100 << "\n";

   length = (int)this->time_flow.size();
if((int)this->time_mass_transport.size()>length)
 length = (int)this->time_mass_transport.size();
if((int)this->time_kinreact.size()>length)
 length = (int)this->time_kinreact.size();
if((int)this->time_equireact.size()>length)
 length = (int)this->time_equireact.size();
if((int)this->time_reactdeact.size()>length)
   length = (int)this->time_reactdeact.size();
if((int)this->time_heat_transport.size()>length)
 length = (int)this->time_heat_transport.size();
if((int)this->time_mechanics.size()>length)
 length = (int)this->time_mechanics.size();

   ofstream out_file (outname.data(),ios::out);
   out_file.precision(6);

    out_file << "Step   Flow   Heat  Mechanics  Transport KinReactions  EquiReactions ReactDeact Total"<< "\n";
    for(i = 0; i < length; i++)
    {
        out_file << i+1 /* this->time_timestep_number[i] */ << "   ";
        tot_zeitschritt = 0;
        //flow
      if((int)time_flow.size() > i)
        ...
        // heat transport
        if((int)time_heat_transport.size() > i)
            help = time_heat_transport[i];
        else
            help = 0.0;
        out_file << help << "   ";
        tot_zeitschritt += help;
        // geomechanics
        if((int)time_mechanics.size() > i)
            help = time_mechanics[i];
        else
            help = 0.0;
        out_file << help << "   ";
        tot_zeitschritt += help;
        //mass transport
        if((int)time_mass_transport.size() > i)
            help = time_mass_transport[i];
        else
            help = 0.0;
        out_file << help << "   ";
        tot_zeitschritt += help;
    ...
    out_file << "\n";
   out_file << "TotalFlow    TotalHeat   TotalGeoMechanics TotalTransport   TotalKinReactions   TotalEquiReactions TotalReactDeact   Total"<< "\n";
   out_file << time_total_flow << "  " << time_total_heat << "  " << time_total_mechanics << "  " << time_total_transport
       << "  " << time_total_kinreact << "  " << time_total_equireact << "  " << time_total_reactdeact << "  " << tot << " " << "  seconds \n";
   out_file <<  time_total_flow/tot*100 << "  " << time_total_heat/tot*100 << "  " << time_total_mechanics/tot*100 << "  "
       << time_total_transport/tot*100 << "  " << time_total_kinreact/tot*100 << "  " << time_total_equireact/tot*100 << "  "
