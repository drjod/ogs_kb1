...
public:
	...
	std::vector <double> time_mass_transport;
	std::vector <double> time_heat_transport;
	std::vector <double> time_mechanics;
	std::vector <double> time_kinreact;
	std::vector <double> time_equireact;
    std::vector <double> time_reactdeact;
	std::vector <unsigned int> time_timestep_number;
	double time_total_flow;
	double time_total_transport;
	double time_total_heat;
	double time_total_kinreact;
	double time_total_equireact;
	double time_total_mechanics;
    double time_total_reactdeact;
	double delta_clocktime;
	clock_t start;
	clock_t end;

	void StopTime(const std::string &name = "", unsigned int akt_timestep = 0);
	void StartTime(void);
